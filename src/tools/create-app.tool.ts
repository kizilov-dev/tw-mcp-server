...
/**
 * ВАЖНО: Все поля в inputSchema являются ОБЯЗАТЕЛЬНЫМИ!
 * LLM должен собрать ВСЕ данные перед вызовом этого tool.
 *
 * Поля, которые LLM должен определить АВТОМАТИЧЕСКИ:
 * - type: тип приложения (frontend/backend)
 * - provider_id: ID VCS провайдера
 ...
 * - build_cmd: команда сборки (можно использовать tool ${ToolNames.GET_DEPLOY_SETTINGS})
 * - run_cmd: команда запуска (можно использовать tool ${ToolNames.GET_DEPLOY_SETTINGS})
 * - system_dependencies: системные зависимости
 * - index_dir: директория с index файлом (можно использовать tool ${ToolNames.GET_DEPLOY_SETTINGS})
 *
 * Поля, которые LLM должен запросить у пользователя:
 * - envs: переменные окружения (КРИТИЧНО!)
 *
 * Поля с значениями по умолчанию:
 * - comment: комментарий
 * - is_auto_deploy: всегда false
 *
 * Поле подтверждения:
 * - confirm: если false - показывается предварительный просмотр, если true - создается приложение
 */
...
const inputSchema = {
  ...
  framework: z
    ...
    .describe(
      "ОБЯЗАТЕЛЬНОЕ ПОЛЕ - фреймворк приложения (React, Vue, Angular, Next.js, Django, Express, Laravel и др.)"
    )
  ...
  envs: z
    .record(z.string(), {
      description:
        "ПЕРЕМЕННЫЕ ОКРУЖЕНИЯ - ОБЯЗАТЕЛЬНО ЗАПРОСИТЬ У ПОЛЬЗОВАТЕЛЯ! Это критически важная информация, которая влияет на работу приложения. Никогда не придумывай значения - они должны быть предоставлены пользователем явно. Не используй переменные окружения, которые не нужны для работы приложения.",
    })
    .describe(
      "ОБЯЗАТЕЛЬНОЕ ПОЛЕ - переменные окружения (запросить у пользователя)"
    )
    .default({}),
  ...
  index_dir: z
    .string({
      description: `Директория с index файлом (для frontend приложений). Использовать tool ${ToolNames.GET_DEPLOY_SETTINGS} для получения настроек по умолчанию для конкретного фреймворка.`,
    })
    .describe(
      "ОБЯЗАТЕЛЬНОЕ ПОЛЕ для frontend приложений - директория с index файлом"
    ),
...
export const createAppTool = {
  ...
  description: `Создать приложение в Timeweb Cloud с автоматическим определением параметров проекта.
    ...
    • Пресет для развертывания (использовать tool ${ToolNames.GET_ALLOWED_PRESETS} для получения доступных пресетов)
    • Команды сборки и запуска (использовать tool ${ToolNames.GET_DEPLOY_SETTINGS} для получения настроек по умолчанию для конкретного фреймворка)
    • Директория с index файлом (для frontend приложений) (использовать tool ${ToolNames.GET_DEPLOY_SETTINGS} для получения настроек по умолчанию для конкретного фреймворка)
    ...
    ❓ ТРЕБУЕТ ВВОДА ПОЛЬЗОВАТЕЛЬЯ:
    • Переменные окружения (envs) - КРИТИЧНО!

    🔍 ПРЕДВАРИТЕЛЬНЫЙ ПРОСМОТР:
    • При первом вызове (confirm: false) показывается детальный просмотр всех собранных данных
    • Пользователь может проверить и при необходимости изменить параметры

    🚫 НЕ ВЫЗЫВАТЬ tool до сбора ВСЕХ данных!
    Перед выполнением автоматически выполняются необходимые проверки и сбор данных.

    ✅ ВАЛИДАЦИЯ: Все поля проходят строгую проверку на корректность и полноту данных.`,
  inputSchema,
  outputSchema,
  handler,
};
 ...
 * РЕЗЮМЕ ВАЛИДАЦИИ:
 *
 * Все поля в inputSchema являются ОБЯЗАТЕЛЬНЫМИ и проходят строгую проверку:
 *
 * 1. Строковые поля: не могут быть пустыми или состоять только из пробелов
 * 2. Числовые поля: должны быть положительными целыми числами
 * 3. Массивы: должны содержать минимум 1 элемент, максимум 50
 * 4. Объекты: должны содержать минимум 1 ключ, максимум 100
 * 5. URL: должны соответствовать формату Git репозитория
 * 6. SHA: должны быть ровно 40 шестнадцатеричных символов
 * 7. Regex валидация для всех полей с ограничениями на символы
 *
 * LLM НЕ сможет вызвать этот tool без предоставления ВСЕХ данных!
 */
